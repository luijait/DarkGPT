...
class DarkGPT:
    ...
    def execute_function_call(self, function_prompts: list, message):
        ...
        try:
            preprocessed_output = response.choices[0].message.function_call.arguments
            processed_output = consultar_dominio_dehashed(eval(preprocessed_output))
        except:
    ...
    def process_history_with_function_output(self, messages: list, function_output: str):
        history = []  # Lista inicial vacía para contener los mensajes formateados y la salida de la función.
        # Agrega la salida de la función al historial.
        history.append({"role": "system", "content": AgentPrompt + function_output})
        for message in messages:
            # Formatea los mensajes basándose en el rol.
            if "USUARIO" in message:
                history.append({"role": "user", "content": message["USUARIO"]})
            elif "ASISTENTE" in message:
                history.append({"role": "assistant", "content": message["ASISTENTE"]})
